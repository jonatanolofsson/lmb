...
double lap(int dim, cost **assigncost, col *rowsol, row *colsol, cost *u,
           cost *v)
...

{
    ...
    pred = new row[dim];     // row-predecessor of column in augmenting/alternating
                             // path.
    ...
    for (f = 0; f < numfree; f++) {
        ...
        up = 0;   // columns in low..up-1 are to be scanned for current minimum,
                 // now none. columns in up..dim-1 are to be considered later to
                 // find new minimum, at this stage the list simply contains all
                 // columns
...
void checklap(int dim, cost **assigncost, col *rowsol, row *colsol, cost *u,
              cost *v) {
    row i;
    col j;
    cost redcost = 0;
    ...
    for (j = 0; j < dim; j++) matched[j] = FALSE;
